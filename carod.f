...
C            STEPV = 0.00058*(ALX+ALY+ALZ)/3.D0
             STEPV = 0.00007D0*(ALX+ALY+ALZ)/3.D0
...
C
       ...
       ISUC = 1 
         ...
         XC_NEW = XC + (2.*RAND()-1.)*STEP
         YC_NEW = YC + (2.*RAND()-1.)*STEP
         ZC_NEW = ZC + (2.*RAND()-1.)*STEP
         ...
         DO J = 1 , NB
          ...
          IF(XN(J).LT.-HALX .OR. XN(J).GT.HALX) RETURN
          IF(YN(J).LT.-HALY .OR. YN(J).GT.HALY) RETURN
          IF(ZN(J).LT.-HALZ .OR. ZN(J).GT.HALZ) RETURN
...
C excluded volume interaction
       ...
       SUBROUTINE ROT(ISUC,STEP)
       IMPLICIT DOUBLE PRECISION (A-H,O-Z)
       PARAMETER (PI=3.141592653589793D0)
       DIMENSION X(1000),Y(1000),Z(1000)
       DIMENSION XTRY(1000),YTRY(1000),ZTRY(1000)
...
C
         ...
         ALPHA = (2.0D0*RAND() - 1.0D0)*(PI*STEP)
         COSA = DCOS(ALPHA)
         SINA = DSIN(ALPHA)
         ICH = 1 + INT(3.0*RAND())
...
C       ROTATE ABOUT Z AXIS
        ...
        DO 150 II = 1 ,  NB
           ...
           YREL = Y(ICHK) - YPIV
           ZREL = Z(ICHK) - ZPIV
           XTRY(II) = XPIV +A11*XREL+A12*YREL+A13*ZREL
           YTRY(II) = YPIV +A21*XREL+A22*YREL+A23*ZREL
           ZTRY(II) = ZPIV +A31*XREL+A32*YREL+A33*ZREL

                TX = XTRY(II)
                TY = YTRY(II)
                TZ = ZTRY(II)

          IF(TX.LT.-HALX .OR. TX.GT.HALX) RETURN
          IF(TY.LT.-HALY .OR. TY.GT.HALY) RETURN
          IF(TZ.LT.-HALZ .OR. TZ.GT.HALZ) RETURN
